...
public class CustomString {
	...
	public void setString(String string) {
		this.isSet = (string != null); 
		//use != null because if it is null then there is no string obj so can't use .equals
	...
	public String remove(String arg) {
        if(!this.isSet || this.myString.equals("")) {
            return "";
        }
        ...
        for(int i=0; i<arg.length(); i++) {
            ...
            if(Character.isLetter(c)){
                newString = newString.replaceAll(Character.toString(c), "");
            }
        }
	...
	public String reverse(String arg) {
		...
		String newString = "";
		for(int j=0; j<this.myString.length(); j++){
			char c = this.myString.charAt(j);
			if(Character.isLetter(c) && arg.indexOf(Character.toLowerCase(c)) >= 0) {
	...
	public String filterLetters(char n, boolean more) {
		...
		if(!this.isSet || this.myString.equals("")) return "";

		for(int j=0; j<this.myString.length(); j++){
			char ch = this.myString.charAt(j);
			char chLowerCase = Character.toLowerCase(ch);
			char nLowerCase = Character.toLowerCase(n);
			if(Character.isLetter(ch)){
				if(more){
					if( chLowerCase < nLowerCase){
						newString += ch;
					}
				} else {
					if(chLowerCase > nLowerCase){

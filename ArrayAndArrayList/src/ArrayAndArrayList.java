...
public class ArrayAndArrayList {
	...
	public ArrayList<Integer> maxArray(int[] array) {
		...
		maxArray.add(array[0]);

		for (int k=1; k<array.length; k++) {
			if (array[k] > maxArray.get(0)) {
				maxArray = new ArrayList<>();
				maxArray.add(array[k]);
			} else if (array[k] == maxArray.get(0)) {
				maxArray.add(array[k]);
			}
		}
		return maxArray;
		...
		if (array.length != 0) {
			...
			for (int e = 0; e < array.length; e++) {
				if (array[e] == 0) {
					...
					int temp = array[e];
					int firstZero = e - snowball;
					array[e] = array[firstZero];
					array[firstZero] = temp;
				}
			}
		}
	}
